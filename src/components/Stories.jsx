...
import { HeartIcon, ClockIcon, HashtagIcon } from "@heroicons/react/20/solid";
          ...
          <Splide options={splideOptions}>
            {news.map((items, index) => (
              <SplideSlide key={index} className="mb-0.5">
                <div className="relative grid items-start gap-4 pb-2 rounded-lg shadow shadow-slate-200 ring-1 ring-slate-200">
                  <div className="flex items-center justify-center">
                    <img
                      src={items.img}
                      alt={`img/stories${index}`}
                  ...
                  <div className="flex items-center justify-between w-full px-4">
                    <div className="flex items-center gap-0.5">
                      ...
                      <span className="text-xs font-bold">{items.like}</span>
                    </div>
                    <div className="flex items-center gap-0.5">
                      <ClockIcon className="icon-style w-4 h-4 text-black" />
                      <span className="text-xs font-bold">{items.time}</span>
                    </div>
                    <div className="flex items-center gap-0.5">
                  ...
                  <div className="grid items-center justify-items-start px-4">
                    ...
                    <p className="text-sm text-justify lg:text-xs ">
                      {truncate(items.text, { length: 175 })}
                      {/* truncate() method of String in lodash is used to truncate the stated string if it is longer than the specified string length. */}
                    </p>
                  </div>
